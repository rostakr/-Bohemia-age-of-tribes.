...
| Central-European oak/birch forest-edge study | Original project code `src/render/trees.ts`; internal project work, no third-party art or textures | Deterministic procedural PlayCanvas geometry batched into one bark mesh + one foliage mesh | 36-tree composition; exact aggregate bounds vary deterministically with terrain placement | Target 10k–24k triangles total; no LOD | Two vertex-colored non-metallic groups: bark and foliage | Phase 1 Bohemian valley forest-edge composition; oak-dominant with birch minority | Integrated on draft PR #28 as explicit WIP candidate; `ADMITTED_MODELS.tree` remains null; exact CI metrics pending; visual, historical and actual-hardware acceptance pending; `artGatePassed=false`. |
