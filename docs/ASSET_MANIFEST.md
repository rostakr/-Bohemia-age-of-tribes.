...
| Generic Boii adult inhabitant study | Original project code `src/render/inhabitant.ts`; internal project work. User reference `B71554C4-303D-4BF0-8AF2-A0BE67B9D8C4.jpeg` used only for broad adult proportions and earth-tone tunic/trouser silhouette; no identity reproduction or source texture | One shared procedural PlayCanvas mesh with vertex colours, cloned to five static benchmark inhabitants | Measured prototype bounds 0.840 × 1.717 × 0.395 m | 1,404 triangles / 910 vertices per shared prototype; no rig/LOD | One vertex-coloured non-metallic material | Boii / Late La Tène generic adult worker readability study | Draft PR #30 run `35293962252` passed 18/18 Node tests, all Phase 0 browser/lifecycle/WebGPU regressions and Phase 1 WebGL2 smoke. Five inhabitants rendered; scene observed 190 draw calls. `ADMITTED_MODELS.inhabitant` remains null; visual, historical and actual-hardware acceptance pending; `artGatePassed=false`. |
