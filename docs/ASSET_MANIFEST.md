...
One Phase 1 dwelling model is imported for evaluation. The GLB/model slots for storehouse, workshop, inhabitant and tree remain empty; project-owned procedural storehouse and workshop candidates are integrated separately and are not represented as admitted production models. On 2026-09-17,
...
| Central-European oak/birch forest-edge study | Original project code `src/render/trees.ts`; internal project work, no third-party art or textures | Deterministic procedural PlayCanvas geometry batched into one bark mesh + one foliage mesh | 36-tree composition; exact aggregate bounds vary deterministically with terrain placement | 16,272 triangles total across 36 trees; no LOD | Two vertex-colored non-metallic groups: bark and foliage | Phase 1 Bohemian valley forest-edge composition; 24 oaks + 12 birches | Draft PR #28 CI run `35293221711` passed 16/16 Node tests, all Phase 0 browser/lifecycle/WebGPU regressions and the Phase 1 WebGL2 smoke; observed 170 draw calls. `ADMITTED_MODELS.tree` remains null; visual, historical and actual-hardware acceptance pending; `artGatePassed=false`. |
| Generic Boii adult inhabitant study | Original project code `src/render/inhabitant.ts`; internal project work. User reference `B71554C4-303D-4BF0-8AF2-A0BE67B9D8C4.jpeg` used only for broad adult proportions and earth-tone tunic/trouser silhouette; no identity reproduction or source texture | One shared procedural PlayCanvas mesh with vertex colours, cloned to five static benchmark inhabitants | ~1.7 m target standing height; exact measured bounds pending CI | Target 1.2k–3k triangles per shared prototype; exact measured count pending CI; no rig/LOD | One vertex-coloured non-metallic material | Boii / Late La Tène generic adult worker readability study | Stacked WIP candidate on `phase1/inhabitant-candidate`; `ADMITTED_MODELS.inhabitant` remains null; structural, visual, historical and actual-hardware acceptance pending; `artGatePassed=false`. |

Exact terrain paths, dimensions, published MD5 and SHA256 are recorded in `assets/source/phase1/terrain-receipt.json`. Exact dwelling generation settings, hashes, geometry and validation are recorded in `assets/source/phase1/dwelling-receipt.json`. Exact procedural storehouse source, deterministic seed, geometry statistics and CI evidence are recorded in `assets/source/phase1/storehouse-receipt.json`. Exact procedural workshop source, deterministic seed, geometry statistics and CI evidence are recorded in `assets/source/phase1/workshop-receipt.json`. Exact forest-edge seed, species-mix target, placement exclusions and QA status are recorded in `assets/source/phase1/tree-study-receipt.json`. Exact inhabitant reference scope, implementation constraints and QA status are recorded in `assets/source/phase1/inhabitant-study-receipt.json`. All images are retained at acquired resolution. The dwelling concept is a 2D design, not a rendered game screenshot. Its preview does not prove final mesh quality.
