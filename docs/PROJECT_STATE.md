...
- Integration discrepancy resolved by direct Git inspection: current `qa/phase4-integration` is `02183da9419d9750070cbba693e18729dbc4b0dd`, a child of merge `be18ce556e0fc25baad758a6256af402314ae42b`. Its conflict-resolution commit changes only `docs/PROJECT_STATE.md` and restores the old Phase 1 record. `src`, `public`, `package.json` and `package-lock.json` are identical to reviewed feature tip `9b03587aed8d8598765e8d5560322321ddf890d3`; gameplay was not lost. Reconcile the integration state document in its existing owner’s next checkpoint.
- Preview delivery blocker: `.github/workflows/deploy-pages.yml` is manual and its build job only permits `refs/heads/main`. It cannot currently publish the Phase 4 integration ref. Development should deliver an isolated, pinned-SHA preview without merging unfinished gameplay into main or overwriting the existing public release. Keep the player URL on `?phase4=1` without debug acceleration; include a build SHA receipt and verify the deployed gather/deposit path.
