...
async function visibleResourceCenter(cdp) {
  return evaluate(cdp, `(() => {
    const nodes = [...document.querySelectorAll('.rts-resource-node')];
...
try {
  ...
  const url = `${previewBase}/?phase3=1&renderer=webgl2&debug=1`;
  ...
  let state = await waitFor(cdp, value =>
    ...
    value.resourceMarkers >= 1 &&
    value.overlayCount === 1 &&
    value.canvasCount === 1,
  'Phase 3 gathering startup', 40_000);
  ...
  state = await waitFor(cdp, value => value.feedback === 'Gather wood' && Number(value?.snapshot?.gatheringUnits) >= 1, 'contextual GATHER feedback', 8_000, 40);
  const beforeDepositScreenshot = await capture(cdp, 'phase3-gather-command-webgl2.png');

  state = await waitFor(cdp, value =>
    Number(value?.snapshot?.woodStockpile) > 0 && Number(value?.snapshot?.woodRemaining) < initialWood,
  'wood deposit', 50_000, 100);
  ...
  const afterDepositScreenshot = await capture(cdp, 'phase3-wood-deposited-webgl2.png');

  await sleep(300);
  const ground = await safeGroundPoint(cdp);
  await rightPointerUp(cdp, ground.x, ground.y);
  await waitFor(cdp, value => value.feedback === 'Move', 'MOVE remains available after gathering', 8_000, 40);
  ...
  state = await waitFor(cdp, value =>
    ...
    Number(value.snapshot.resourceNodes) >= 1 &&
    Number(value.snapshot.woodStockpile) === 0 &&
    value.overlayCount === 1 && value.canvasCount === 1,
  'Phase 3 remount', 40_000);
