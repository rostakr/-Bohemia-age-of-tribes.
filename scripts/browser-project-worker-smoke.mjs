...
const baseUrl = `http://${host}:${previewPort}/?renderer=webgl2&debug=1&worker=project`;
...
const stateExpression = `(() => {
  ...
  return {
    ...
    workerResourceLoaded: resources.some(name => name.includes('boii_adult_worker_project.glb')),
    diagnostics,
  };
})()`;
...
const profileDir = mkdtempSync(join(tmpdir(), 'bohemia-project-worker-chrome-'));
let chrome;
let cdp;
...
try {
  ...
  while (Date.now() < deadline) {
    ...
    if (state?.errorHidden === false || state?.status === 'Renderer unavailable') {
      throw new Error(`Project worker runtime error: ${JSON.stringify(state)}`);
    }
  ...
  if (!healthyState) throw new Error(`Timed out waiting for project worker scene: ${JSON.stringify(state)}`);
  ...
  if (
    ...
    throw new Error(`Project worker scene became unhealthy: ${JSON.stringify(finalState)}`);
  }

  const capture = await cdp.send('Page.captureScreenshot', { format: 'png', fromSurface: true, captureBeyondViewport: false });
  const screenshotPath = resolve(artifactsDir, 'project-worker-webgl2-1920x1080.png');
  writeFileSync(screenshotPath, Buffer.from(capture.data, 'base64'));

  console.log('Project adult-worker browser smoke passed.');
  console.log(JSON.stringify({
    tick: d.tick,
    structures: d.structures,
    inhabitants: d.inhabitants,
    workerResourceLoaded: finalState.workerResourceLoaded,
