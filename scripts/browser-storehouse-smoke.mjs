...
const profileDir = mkdtempSync(join(tmpdir(), 'bohemia-storehouse-chrome-'));
let chrome;
let cdp;
...
try {
  ...
  if (
    ...
    finalState?.diagnostics?.failed !== false ||
    finalState?.diagnostics?.deviceLost !== false ||
    Number(finalState?.diagnostics?.structures) !== 3
  ) {
  ...
  const screenshotPath = resolve(artifactsDir, 'storehouse-supplied-webgl2-1920x1080.png');
  writeFileSync(screenshotPath, Buffer.from(capture.data, 'base64'));

  console.log('Supplied storehouse WebGL2 smoke passed.');
  console.log(JSON.stringify({
    tick: finalState.diagnostics.tick,
    structures: finalState.diagnostics.structures,
    inhabitants: finalState.diagnostics.inhabitants,
    trees: finalState.diagnostics.trees,
    drawCalls: finalState.diagnostics.drawCalls,
    storehouseDiagnosticBeforeAdmission: finalState.diagnostics.storehouseCandidate,
