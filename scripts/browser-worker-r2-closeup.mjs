...
const baseUrl = `http://${host}:${previewPort}/?renderer=webgl2&debug=1&scene=worker-r2-preview`;
...
const profileDir = mkdtempSync(join(tmpdir(), 'bohemia-worker-r2-closeup-'));
let chrome;
let cdp;
...
try {
  ...
  while (Date.now() < deadline) {
    ...
    if (state?.errorHidden === false || state?.status === 'Renderer unavailable') throw new Error(`Worker R2 close-up runtime error: ${JSON.stringify(state)}`);
    const d = state?.diagnostics;
    if (
      state?.status === 'WEBGL2 · Worker R2 preview running' && state.canvasCount === 1 &&
      d?.renderer === 'webgl2' && d?.failed === false && d?.deviceLost === false &&
      d?.milestone === 'phase-1-worker-r2-preview' && d?.workerPreview === true && d?.workerLoaded === true &&
  ...
  if (!healthy) throw new Error(`Timed out waiting for worker R2 close-up: ${JSON.stringify(state)}`);
  ...
  const screenshotPath = resolve(artifactsDir, 'worker-r2-closeup-webgl2-1200x1200.png');
  writeFileSync(screenshotPath, Buffer.from(capture.data, 'base64'));
  console.log('Worker R2 close-up smoke passed.');
  console.log(JSON.stringify({ tick: healthy.diagnostics.tick, workerLoaded: true, drawCalls: healthy.diagnostics.drawCalls, fps: healthy.diagnostics.fps, frameMs: healthy.diagnostics.frameMs, screenshot: screenshotPath }, null, 2));
} finally {
  cdp?.close();
