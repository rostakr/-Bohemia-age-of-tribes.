...
const baseUrl = `http://${host}:${previewPort}/?renderer=webgl2&debug=1&candidate=phase1`;
...
const profileDir = mkdtempSync(join(tmpdir(), 'bohemia-workshop-project-chrome-'));
let chrome;
let cdp;
...
try {
  ...
  while (Date.now() < deadline) {
    ...
    if (state?.errorHidden === false || state?.status === 'Renderer unavailable') {
      throw new Error(`Project-workshop runtime error: ${JSON.stringify(state)}`);
    }
    ...
    if (
      ...
      Number(diagnostics.workshopTriangles) === 22_540 &&
      diagnostics.inhabitantCandidate === 'glb-candidate' &&
  ...
  // Capture the same combined candidate at all existing art-inspection views.
  for (const view of ['settlement', 'river']) {
    ...
    writeFileSync(resolve(artifactsDir, `phase1-completion-${view}-1920x1080.png`), Buffer.from(capture.data, 'base64'));
  }
  ...
  if (
    ...
    diagnostics?.storehouseCandidate !== 'project-owned-glb' ||
    Number(diagnostics?.storehouseTriangles) !== 15_550 ||
    diagnostics?.workshopCandidate !== 'project-owned-glb-preview' || Number(diagnostics?.workshopTriangles) !== 22_540 ||
    diagnostics?.inhabitantCandidate !== 'glb-candidate'
  ) {
  ...
  const capture = await cdp.send('Page.captureScreenshot', { format: 'png', fromSurface: true, captureBeyondViewport: false });
  const screenshotPath = resolve(artifactsDir, 'phase1-project-workshop-1920x1080.png');
  writeFileSync(screenshotPath, Buffer.from(capture.data, 'base64'));

  console.log('Project-workshop Phase 1 browser smoke passed.');
  console.log(JSON.stringify({
    tick: diagnostics.tick,
    structures: diagnostics.structures,
    workshopPreview: diagnostics.workshopPreview,
