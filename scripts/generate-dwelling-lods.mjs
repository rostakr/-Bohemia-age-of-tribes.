...
function imageBytes(doc, imageIndex) {
  ...
  if (image?.mimeType !== 'image/webp' || image.bufferView === undefined) {
    throw new Error('Dwelling LOD generation requires embedded WebP source textures');
  }
  const view = doc.json.bufferViews[image.bufferView];
  const start = view.byteOffset ?? 0;
  return Buffer.from(doc.bin.subarray(start, start + view.byteLength));
}
...
function buildGlb(source, geometry, label) {
  const chunks = [
    typedBuffer(geometry.indices),
    typedBuffer(geometry.positions),
    imageBytes(source, 0),
    imageBytes(source, 1),
    typedBuffer(geometry.uvs),
    typedBuffer(geometry.normals),
  ].map(buffer => pad4(buffer));

  let byteOffset = 0;
  const bufferViews = chunks.map(buffer => {
    const view = { buffer: 0, byteOffset, byteLength: buffer.length };
    byteOffset += buffer.length;
    return view;
  });

  const positionBounds = minMax(geometry.positions, 3);
  const uvBounds = minMax(geometry.uvs, 2);
  let maxIndex = 0;
  for (const index of geometry.indices) maxIndex = Math.max(maxIndex, index);

  const json = structuredClone(source.json);
  json.asset = { version: '2.0', generator: `BOHEMIA deterministic dwelling ${label} vertex-cluster LOD generator` };
  json.bufferViews = bufferViews;
  json.buffers = [{ byteLength: byteOffset }];
  json.accessors = [
    {
      componentType: geometry.indices.BYTES_PER_ELEMENT === 2 ? 5123 : 5125,
      type: 'SCALAR', bufferView: 0, count: geometry.indices.length, max: [maxIndex], min: [0],
    },
    {
      componentType: 5126, type: 'VEC3', bufferView: 1, count: geometry.positions.length / 3,
      max: positionBounds.max, min: positionBounds.min,
    },
    {
      componentType: 5126, type: 'VEC2', bufferView: 4, count: geometry.uvs.length / 2,
      max: uvBounds.max, min: uvBounds.min,
    },
    {
      componentType: 5126, type: 'VEC3', bufferView: 5, count: geometry.normals.length / 3,
      max: [1, 1, 1], min: [-1, -1, -1],
    },
  ];
  json.images = [
    { bufferView: 2, mimeType: 'image/webp' },
    { bufferView: 3, mimeType: 'image/webp' },
  ];
...
export function generateDwellingLods(inputPath, outputDir) {
  const source = parseGlb(inputPath);
  const primitive = source.json.meshes?.[0]?.primitives?.[0];
  if (!primitive || primitive.mode !== 4) throw new Error('Expected one TRIANGLES primitive');
  if (source.json.meshes.length !== 1 || source.json.materials?.length !== 1 || source.json.images?.length !== 2) {
    throw new Error('Dwelling source structure changed; review the LOD generator before proceeding');
  }

  const positions = accessorArray(source, primitive.attributes.POSITION);
  const uvs = accessorArray(source, primitive.attributes.TEXCOORD_0);
  const indices = accessorArray(source, primitive.indices);
  const configs = [
    ['lod1', 0.015, 0.05],
    ['lod2', 0.025, 0.05],
  ];
  ...
  const stats = [];
  for (const [label, voxel, uvStep] of configs) {
    const geometry = clusterGeometry(positions, uvs, indices, voxel, uvStep);
    const bytes = buildGlb(source, geometry, label);
    const outputPath = `${outputDir}/boii_dwelling_rectangular_${label}.glb`;
    ...
    stats.push(entry);
    console.log(`dwelling-${label}-stats ${JSON.stringify(entry)}`);
  }
  return stats;
}

const [, , inputPath, outputDir] = process.argv;
if (inputPath) generateDwellingLods(inputPath, outputDir ?? 'public/assets/buildings');
