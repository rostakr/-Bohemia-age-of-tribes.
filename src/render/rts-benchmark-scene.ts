...
export class RtsBenchmarkScene implements RuntimeScene {
  ...
  async enter(app: Application): Promise<void> {
    ...
    const spawns: UnitSpawn[] = initialEntities.map((entity, index) => {
      ...
      return {
        ...
        ...(this.milestone === 'phase-4' && this.gatheringQaFast ? { speed: 30 } : {}),
      };
    });
    const pathBudget = scalable
    ...
    if (this.milestone === 'phase-4') {
      ...
      const trees = this.findResourceTrees(app)
        .sort((a, b) => {
          ...
          const ad = Math.hypot(ap.x, ap.z); const bd = Math.hypot(bp.x, bp.z);
      ...
      this.gatherLoop = new GatherLoop(this.economy, spawns.map(spawn => ({
        id: spawn.id,
        owner: spawn.owner,
        position: spawn.position,
        carryCapacity: this.gatheringQaFast ? 2 : 10,
        gatherRatePerSecond: this.gatheringQaFast ? 20 : 2,
      })));
      this.gatherCoordinator = new GatherCoordinator(
        navigation,
        this.simulation,
        this.economy,
        this.gatherLoop,
    ...
    this.controller = new RtsController(
      ...
      this.unitEntities,
      () => this.tick,
      this.gatherCoordinator && this.economy
        ? { coordinator: this.gatherCoordinator, resourceEntities: this.resourceEntities }
        : undefined,
    );
  }
  ...
  private findResourceTrees(app: Application): Entity[] {
    ...
    for (let index = 1; index <= 64; index++) {
      const entity = app.root.findByName(`Procedural deciduous tree ${index}`) as Entity | null;
      if (entity && !seen.has(entity)) { seen.add(entity); result.push(entity); }
    }
    for (let index = 0; index <= 64; index++) {
      const entity = app.root.findByName(`Deciduous tree ${index}`) as Entity | null;
      if (entity && !seen.has(entity)) { seen.add(entity); result.push(entity); }
    }
    return result;
  }
  ...
  fixedUpdate(dtSeconds: number, tick: number): void {
    ...
    this.gatherCoordinator?.fixedUpdate(dtSeconds, tick);
  }
  ...
  diagnostics(): SceneDiagnostics {
    ...
    const gathering = this.gatherCoordinator?.metrics();
    return {
      ...base,
      ...
      phase4DebugUnits: this.milestone === 'phase-4' ? countOrDefault(this.debugUnitCount, 120) : 0,
      phase4QaFast: this.milestone === 'phase-4' && this.gatheringQaFast,
      resourceNodes: this.gatherCoordinator?.resourceState().length ?? 0,
      activeGatherOrders: gathering?.activeGatherOrders ?? 0,
      gatheringWorkers: gathering?.gatheringWorkers ?? 0,
      returningWorkers: gathering?.returningWorkers ?? 0,
      ...
      gatherFailure: gathering?.lastGatherFailure ?? '',
    };
  }
  ...
  destroy(): void {
    ...
    this.gatherCoordinator?.destroy();
    this.gatherCoordinator = undefined;
    this.gatherLoop?.destroy();
    this.gatherLoop = undefined;
