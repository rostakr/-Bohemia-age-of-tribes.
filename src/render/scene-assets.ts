...
export class SceneAssets {
  ...
  constructor(private readonly app: Application, private readonly assetResolver: AssetResolver = resolveAsset) {}
  ...
  load(path: string, type: 'container' | 'texture'): Promise<Asset> {
    ...
    const promise = new Promise<Asset>((resolve, reject) => {
      const asset = new Asset(path, type, { url: this.assetResolver(path) });
      this.owned.add(asset);
      const timeout = globalThis.setTimeout(() => fail('Asset load timed out'), 45_000);
      const cleanup = () => {
        globalThis.clearTimeout(timeout);
        asset.off('load', success);
        asset.off('error', fail);
        this.pending.delete(cancel);
      };
      const fail = (error: unknown) => {
        cleanup();
        reject(new Error(`${path}: ${String(error)}`));
      };
...
export function instantiateAtHeight(resource: ContainerResource, name: string, height: number): Entity {
  ...
  if (!first) {
    root.destroy();
    throw new Error(`${name}: no renderable meshes`);
  }
