...
import { resolveAsset } from '../assets/resolve-asset';
...
export class SceneAssets {
  ...
  load(path: string, type: 'container' | 'texture'): Promise<Asset> {
    ...
    const logicalPath = path.startsWith('assets/') ? path.slice('assets/'.length) : path;
    const key = `${type}:${logicalPath}`;
    const cached = this.cache.get(key);
    if (cached) return cached;
    const promise = new Promise<Asset>((resolve, reject) => {
      const asset = new Asset(logicalPath, type, { url: resolveAsset(logicalPath) });
      ...
      const fail = (error: unknown) => { cleanup(); reject(new Error(`${logicalPath}: ${String(error)}`)); };
