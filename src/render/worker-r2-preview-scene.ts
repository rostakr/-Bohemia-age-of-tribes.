...
export const WORKER_R2_PATH = 'characters/boii_adult_worker_r2.glb';

/** QA-only close-up scene for the supplied-source worker R2 candidate. */
export class WorkerR2PreviewScene implements RuntimeScene {
  ...
  async enter(app: Application): Promise<void> {
    if (this.root) throw new Error('Worker R2 preview scene already entered');
    this.destroyed = false;
    this.app = app;
    this.root = new Entity('Worker R2 QA close-up');
    app.root.addChild(this.root);
    this.assets = new SceneAssets(app);

    app.scene.ambientLight = new Color(0.46, 0.48, 0.50);

    const camera = new Entity('Worker R2 preview camera');
    ...
    const key = new Entity('Worker R2 preview key');
    ...
    const fill = new Entity('Worker R2 preview fill');
    ...
    const ground = new Entity('Worker R2 preview ground');
    ground.addComponent('render', { type: 'plane' });
    ground.setLocalScale(4, 1, 4);
    this.groundMaterial = new StandardMaterial();
    this.groundMaterial.name = 'Worker R2 preview neutral ground';
    ...
    const resource = await this.assets.model(WORKER_R2_PATH);
    if (this.destroyed || !this.root) return;
    const worker = instantiateAtHeight(resource, 'Boii adult worker R2 candidate', 1.72);
  ...
  diagnostics(): SceneDiagnostics {
    return {
      milestone: 'phase-1-worker-r2-preview',
      artGatePassed: false,
      workerPreview: true,
      workerLoaded: this.loaded,
      workerPath: WORKER_R2_PATH,
