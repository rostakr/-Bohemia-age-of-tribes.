...
export function createWorkshopGeometry(): WorkshopGeometry {
  ...
  // Packed working floor, slightly proud of the terrain to prevent z-fighting.
  addOrientedBox(earth, [0, 0.035, 0], 2.45, 0.035, 1.42);
  ...
  addOrientedBox(timber, [0.55, 0.91, -0.55], 1.25, 0.09, 0.36);
  ...
  for (const x of [-1.55, 1.75]) {
    ...
    addCylinder(timber, [x + 0.42, 0.05, 1.08], [x + 0.16, 0.72, 1.08], 0.06, 8);
    addOrientedBox(timber, [x, 0.75, 0.90], 0.48, 0.055, 0.24);
  }
  ...
  for (const side of [-1, 1]) {
    for (let i = 0; i < 600; i++) {
      ...
      const y = ridgeY - t * rise + (random() - 0.5) * 0.035;
      const length = 0.34 + random() * 0.26;
    ...
    for (let i = 0; i < 64; i++) {
      ...
      const z = side * (halfRun + 0.02 + (random() - 0.5) * 0.06);
      const y = eaveY + (random() - 0.5) * 0.06;
      addCylinder(thatch, [x, y, z], [x, y - 0.24 - random() * 0.09, z + side * 0.12], 0.01, 8);
    }
  }

  return { timber, thatch, earth, iron };
}
...
export function createBoiiWorkshop(app: Application): ProceduralWorkshop {
  ...
  definitions[3]![1].metalness = 0.65;
  definitions[3]![1].update();
